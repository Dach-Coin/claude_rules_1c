...
node tests/skills/runner.mjs --json report.json                 # JSON-отчет
```
...
|---|---|---|
| `name` | да | Название теста (отображается в отчете) |
| `input` | нет | JSON-объект, передается навыку через temp-файл |
