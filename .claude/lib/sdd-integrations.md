...
| Framework | Detection |
|-----------|-----------|
| **claude-memory** | Always available in Claude Code. Check for `CLAUDE.md` in project root |
...
## 1. Claude Auto Memory

A built-in persistent memory system in Claude Code that stores context, patterns, and decisions across sessions via structured markdown files.
...
| File | Purpose |
|------|---------|
| `MEMORY.md` | Index file, always loaded into context (max 200 lines) |
| `patterns.md` | Code patterns and architectural decisions |
| `debugging.md` | Recurring bugs and their solutions |
| `project.md` | Key file paths, project structure, conventions |
| *(topic files)* | Any domain-specific notes linked from `MEMORY.md` |
...
| Agent | Claude Memory Usage |
|-------|---------------------|
| **planner** | Read `MEMORY.md` for project context before planning |
| **architect** | Save architectural decisions to `patterns.md` |
| **developer** | Read patterns, save newly discovered code conventions |
| **analytic** | Reference stored domain knowledge from memory files |
| **code-reviewer** | Document recurring issues in `debugging.md` |
| **metadata-manager** | Save metadata structure findings to `project.md` |
| **doc-writer** | Use memory files as source for documentation |
...
| **metadata-manager** | Create spec deltas for new metadata objects in `openspec/changes/` |
...
| **metadata-manager** | Verify new metadata objects align with `constitution.md` and `boundaries.md` |
...
| **metadata-manager** | Use `set_task_status` after metadata creation/validation completes |
