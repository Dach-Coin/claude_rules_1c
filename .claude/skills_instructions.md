...
Публикация ИБ через Apache, статус и остановка веб-сервера, снятие публикации, e2e-тестирование через веб-клиент (Playwright). Связка, заменяющая прежний локальный `deploy-and-test`: `db-update` (конфигурация БД) -> `web-publish` -> `web-test`.
