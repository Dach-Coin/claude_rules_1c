...
<!-- Scope: this file is the single source of truth for MCP tool selection and tool-driven workflows.
     Coding standards live in project_rules.md. Skills live in skills_instructions.md. -->

## Scope

- This file describes the three MCP-class sources available in the current environment and maps engineering tasks to the right tool.
...
Three sources cover all tool-driven work on a 1C codebase:

| Source | Purpose |
|---|---|
| **rlm-tools-bsl** | Exploration of the 1C codebase (BSL modules, XML metadata, call graph, cross-references) through a Python sandbox session |
| **1c-syntax** | Platform reference (built-in functions, methods, objects, syntax validation, autocomplete) |
| **claude-code-bsl-lsp** | LSP integration for BSL Language Server — real-time diagnostics, go-to-definition, find-references, rename |
...
## 3. claude-code-bsl-lsp — diagnostics and navigation

### Purpose

LSP integration over `BSL Language Server`. Binds to `.bsl` / `.os` files and provides the usual IDE-grade capabilities.
...
| Diagnose a module after writing | `claude-code-bsl-lsp` | — | — |
...
6. Diagnostics pass: `claude-code-bsl-lsp`. Fix errors; at most three iterations on style warnings.
...
5. Diagnostics: `claude-code-bsl-lsp`.
...
5. Apply rewrites.
6. Diagnostics: `claude-code-bsl-lsp`.
7. `rlm_end`.
...
3. After writing or modifying BSL code, a `claude-code-bsl-lsp` diagnostics pass is required.
