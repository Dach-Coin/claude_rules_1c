---
name: bsl-anti-patterns
description: Каталог анти-паттернов и оптимизированных паттернов BSL/1С (критические, высокие, средние, архитектурные). Используй при код-ревью, рефакторинге, анализе производительности, планировании доработок - когда нужно проверить код на известные проблемы (запрос в цикле, доступ через точку, подзапросы в SELECT, виртуальные таблицы, кеширование, O(n^2), вложенность) и взять готовую замену. Содержит confidence-scoring для отчетов ревью.
argument-hint: (no arguments)
allowed-tools: []
---

# /bsl-anti-patterns - каталог анти-паттернов 1C/BSL

This skill consolidates all anti-patterns, performance issues, and best practice violations for 1C development. Reference this skill instead of duplicating examples. Заменяет утраченный автоматический анализатор логики/производительности - используется как ручной чек-лист в ревью/рефакторинге/оптимизации.
...
**SSL Methods Reference:** See `.claude/rules/project_rules.md` -> "Reference Attribute Access" section.
...
```bsl
// MEDIUM: O(n^2) nested loop search
...
| O(n^2) loops | MEDIUM | Nested loops searching for matches |
